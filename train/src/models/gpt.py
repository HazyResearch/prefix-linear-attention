...
class GPTLMHeadModel(GPTPreTrainedModel, GenerationMixin, NaiveGenerationMixin):
    def __init__(self, config: GPT2Config, process_group=None, device=None, dtype=None):
        ...
        # SA: code backwards compatibility
        try:
            if config.mixer['_target_'] == "src.models.mixers.base_conv.BaseConvWithSiLU4":
                config.mixer['_target_'] = "based.models.mixers.convolution.BaseConv"   
            if config.alt_mixer['_target_'] == "src.models.mixers.linear_attention.LinearAttention":
                config.alt_mixer['_target_'] = "based.models.mixers.linear_attention.LinearAttention"
            if config.alt_mixer_2['_target_'] == "src.models.mixers.slide_fa2.SlidingsMHA":
                config.alt_mixer_2['_target_'] = "based.models.mixers.slide_attention.SlidingAttention"
            if 'feature_map' in config.alt_mixer and config.alt_mixer['feature_map']['_target_'] == "src.models.mixers.linear_attention.TaylorExp":
                config.alt_mixer['feature_map']['_target_'] = "based.models.mixers.linear_attention.TaylorExp"
            if 'do_update' in config.alt_mixer_2:
                del config.alt_mixer_2['do_update']
        except:
            pass
        # End of adjustment
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        # SA compatibility. Rename "conv1" to "conv".
        for k in list(state_dict.keys()):
            if "conv1" in k:
                state_dict[k.replace("conv1", "conv")] = state_dict.pop(k)
