...
class SequenceModel(LightningModule):
    ...
    def __init__(self, cfg, model_cfg=None):
        ...
        self.mlm_probability = getattr(self.model_cfg.config, 'mlm_probability', 0.15) 
        self.mask_token_id = getattr(self.model_cfg.config, 'mask_token_id', 50263) 
...
class SequenceLMModel(SequenceModel):
    ...
    def step(self, batch: Any, is_train=True, batch_idx=None):
        ...
        if self.data_type == 'mixed':
            enc_labels = x[:,:enc_length].clone()
            probability_matrix = torch.full(enc_labels.shape, self.mlm_probability)
            masked_indices = torch.bernoulli(probability_matrix).bool()
            enc_labels[~masked_indices] = -100  # If it's not a mask, don't compute loss
            dec_labels = y[:,enc_length:].clone()
            y = torch.concat([enc_labels, dec_labels], dim=-1)
            try:
                x[:,:enc_length][masked_indices.numpy()] = self.mask_token_id
            except:
                mask = torch.zeros_like(x[:,:enc_length], dtype=torch.bool)
                mask[masked_indices] = True
                x[:,:enc_length][mask] = self.mask_token_id
