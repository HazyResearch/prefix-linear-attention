    ...
    def custom_enc_dec_encode(
        ...
        if not is_prefill:
            ...
            if seqlen < enc_length:
                ...
                # pad with the first prefill_size_pad of the context
                # padding = encodings[:, : min(prefill_size_pad, len(encodings))]
                # padding = padding.repeat(1, prefill_size_pad // len(padding) + 1)[:, :prefill_size_pad]
                encodings = torch.cat(
                    [
                        padding,
                        encodings
