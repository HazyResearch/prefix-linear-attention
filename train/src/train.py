...
def log_predictions(predictions, datamodule, num_batches, name):
        ...
        for i, (batch, preds) in enumerate(zip(datamodule.test_dataloader(), predictions)):
            ...
            orig_labels = metadata_batch['orig_labels']
            ...
            try:
                # last_1024_tokens = label_ids_batch[:, -1024:]
                # label_probs = probs[:, -1024:].gather(dim=-1, index=last_1024_tokens.unsqueeze(-1)).squeeze(-1)
                # zeros_tensor = torch.zeros((label_ids_batch.size(0), 1024), dtype=probs.dtype)
                # label_probs = torch.cat((zeros_tensor, label_probs), dim=-1)

                label_probs = probs.gather(dim=-1, index=orig_labels.unsqueeze(-1)).squeeze(-1)
            except:
                breakpoint()
        ...
        with tempfile.TemporaryDirectory(prefix="/work/sabri_data") as tmp_dir:
            ...
            artifact = wandb.Artifact(
                f"run-{wandb.run.id}", type="feather"
            )
...
def train(config: DictConfig) -> Optional[float]:
    ...
    do_predict = getattr(config, "do_predict", True)
    ...
    # Log predictions
    if do_predict:
        log.info(f"Starting predictions! Batches: {config.datamodule.num_predict_batches_eval}")
