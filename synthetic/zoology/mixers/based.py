        ...
        *args, **kwargs
        ...
        if self.train_view == "linear":
            if causal_dot_product is not None and self.causal:
                    v = causal_dot_product(q.contiguous(), k.contiguous(),v.contiguous(),)
                    z = 1 / (
                        torch.einsum(
                            "bhld,bhld->bhl", q, k.cumsum(2)
                        ) + self.eps)
                    y = v * z[..., None]
            ...
            if self.causal:
                cumsum_matrix = torch.tril(torch.ones((l, l))).to(q.device)
                A_qk = torch.einsum("bhnd,bhmd->bhnm", q, k) * cumsum_matrix
            else:
                A_qk = torch.einsum("bhnd,bhmd->bhnm", q, k)

            out = torch.einsum("bhnm,bhme->bhne", A_qk, v)
